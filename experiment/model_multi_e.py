...
class HELPv3e(torch.nn.Module):
    def __init__(self, num_edge_classes):
        ...
        for i in range(cm0_num):
            conv = HeteroConv({
                ('human', 'interact', 'object'): GATConv(in_channels=(-1, -1), out_channels=cm0_dim, add_self_loops=False),
                ('object', 'interacted_by', 'human'): GATConv(in_channels=(-1, -1), out_channels=cm0_dim, add_self_loops=False),
            }, aggr='sum')
            self.conv_module_0.append(conv)
        self.conv_module_0 = torch.nn.ModuleList(self.conv_module_0)

        self.edge_mlp1 = Linear(-1, 32)
        self.pred_mlp1_0 = Linear(-1, 512)
        self.pred_mlp2_0 = Linear(-1, 512)
    ...
    def forward(self, x_dict, edge_index_dict, edge_attr_dict, hbatch, obatch):
        ...
        edge_logits1 = F.relu(self.pred_mlp1_0(graph_emb))
        edge_logits2 = F.relu(self.pred_mlp2_0(graph_emb))
        edge_logits1 = F.softmax(self.pred_mlp1(edge_logits1), dim=1)
        edge_logits2 = F.softmax(self.pred_mlp2(edge_logits2), dim=1)
